...
Transport.prototype.handleRequest = function (req) {
  this.log.trace('setting request', req.method, req.url);
  this.req = req;
...
Transport.prototype.onForcedDisconnect = function () {
  if (!this.disconnected) {
    this.log.debug('transport end by forced client disconnection');
    if (this.open) {
...
Transport.prototype.setCloseTimeout = function () {
  if (!this.closeTimeout) {
    ...
    this.closeTimeout = setTimeout(function () {
      self.log.trace('fired close timeout for client', self.id);
      self.closeTimeout = null;
      self.end('close timeout');
    }, this.manager.get('close timeout') * 1000);

    this.log.trace('set close timeout for client', this.id);
  }
};
...
Transport.prototype.clearCloseTimeout = function () {
  if (this.closeTimeout) {
    ...
    this.log.trace('cleared close timeout for client', this.id);
  }
};
...
Transport.prototype.setHeartbeatTimeout = function () {
  if (!this.heartbeatTimeout && this.manager.enabled('heartbeats')) {
    ...
    this.heartbeatTimeout = setTimeout(function () {
      self.log.trace('fired heartbeat timeout for client', self.id);
      self.heartbeatTimeout = null;
      self.end('heartbeat timeout');
    }, this.manager.get('heartbeat timeout') * 1000);

    this.log.trace('set heartbeat timeout for client', this.id);
  }
};
...
Transport.prototype.clearHeartbeatTimeout = function () {
  if (this.heartbeatTimeout && this.manager.enabled('heartbeats')) {
    ...
    this.log.trace('cleared heartbeat timeout for client', this.id);
  }
};
...
Transport.prototype.setHeartbeatInterval = function () {
  if (!this.heartbeatInterval && this.manager.enabled('heartbeats')) {
    ...
    this.log.trace('set heartbeat interval for client', this.id);
  }
};
...
Transport.prototype.heartbeat = function () {
  if (this.open) {
    this.log.trace('emitting heartbeat for client', this.id);
...
Transport.prototype.onMessage = function (packet) {
  ...
  if ('heartbeat' == packet.type) {
    this.log.trace('got heartbeat packet');
    ...
    if ('disconnect' == packet.type && packet.endpoint == '') {
      this.log.trace('got disconnection packet');
    ...
    if (packet.id && packet.ack != 'data') {
      this.log.trace('acknowledging packet automatically');
...
Transport.prototype.clearHeartbeatInterval = function () {
  if (this.heartbeatInterval && this.manager.enabled('heartbeats')) {
    ...
    this.log.trace('cleared heartbeat interval for client', this.id);
  }
};
...
Transport.prototype.end = function (reason) {
  if (!this.disconnected) {
    this.log.debug('transport end (' + reason + ')');
...
Transport.prototype.discard = function () {
  this.log.trace('discarding transport');
  this.discarded = true;
  this.clearTimeouts();
  this.clearHandlers();
...
Transport.prototype.writeVolatile = function (msg) {
  if (this.open) {
    if (this.drained) {
      ...
      this.log.trace('ignoring volatile packet, buffer not drained');
    }
  } else {
    this.log.trace('ignoring volatile packet, transport not open');
  }
};
