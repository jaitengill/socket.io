...
function Manager (server, options) {
  ...
  this.log.debug('socket.io started');
};
...
Manager.prototype.onDisconnect = function (id) {
  ...
  if (this.sockets.sockets[id]) {
    this.log.debug('manager deleting socket id: ' + id);
...
Manager.prototype.handleRequest = function (req, res) {
  ...
  if (data.static || !data.transport && !data.protocol) {
    if (data.static && this.enabled('browser client')) {
      ...
      this.log.trace('unhandled socket.io url');
    }
  ...
  if (data.protocol != protocol) {
    ...
    this.log.trace('client protocol version unsupported');
  } else {
    if (data.id) {
...
Manager.prototype.handleUpgrade = function (req, socket, head) {
  ...
  if (!data) {
    if (this.enabled('destroy upgrade')) {
      socket.end();
      this.log.trace('destroying non-socket.io upgrade');
    }
...
Manager.prototype.handleHandshake = function (data, req, res) {
  ...
  this.authorize(handshakeData, function (err, authorized, newData) {
    ...
    if (authorized) {
      ...
      self.log.trace('handshake authorized', id);
    } else {
      writeErr(403, 'handshake unauthorized');
      self.log.trace('handshake unauthorized');
    }
  })
};
...
Manager.prototype.authorize = function (data, fn) {
  if (this.get('authorization')) {
    ...
    this.get('authorization').call(this, data, function (err, authorized) {
      self.log.trace('client ' + authorized ? 'authorized' : 'unauthorized');
      fn(err, authorized);
    });
  } else {
    this.log.trace('client authorized');
    fn(null, true);
  }
